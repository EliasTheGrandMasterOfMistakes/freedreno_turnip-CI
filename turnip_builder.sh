...
EOF
	...
	meson build-android-aarch64 --cross-file "$workdir"/mesa-main/android-aarch64 -Dbuildtype=release -Doptimization=3 -Dperfetto=true -Dplatforms=android -Dgallium-drivers= -Dplatform-sdk-version=$sdkver -Dandroid-stub=true -Dvulkan-beta=true -Dvulkan-drivers=freedreno -Dfreedreno-kmds=kgsl -Db_lto=false | tee "$workdir"/meson_log
