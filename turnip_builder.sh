...
EOF
	...
	meson build-android-aarch64 --cross-file "$workdir"/mesa-main/android-aarch64 -Dbuildtype=release -Doptimization=3 -Dplatforms=android -Dgallium-drivers= -Dplatform-sdk-version=$sdkver -Dandroid-stub=true -Dshader-cache-max-size=5G -Dzstd=enabled -Dxmlconfig=enabled -Dandroid-strict=false -Dvulkan-beta=true -Dvulkan-drivers=freedreno -Dfreedreno-kmds=kgsl -Db_lto=false | tee "$workdir"/meson_log
